...
class ColorGame extends React.Component {
    ...
    pickColorPair() {
        ...
        console.log('game redux', this.props.game.gameStarted);
    ...
    countDown() {
        const stopId = setInterval(() => this.props.startTimer(this.props.timer.timeLeft - 1), 1000);
    ...
    render() {
        ...
        return (
            <div className="container">
                ...
                <h3>{this.props.timer.timerId}</h3>
                <h3>{this.props.timer.timeLeft}</h3>
                <button>{this.props.timer.timeLeft}</button>

                <button className='startbtn' onClick={this.pickColorPair}>{this.props.timer.timeLeft === 0 ? "Replay " : "Start"}</button>
